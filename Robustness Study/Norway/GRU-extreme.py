...
def main():
    ...
    result_root = os.path.join(".", "results", f"{EXPERIMENT_NAME}_norway12_gru_fixed_only_model")
    base_dirs = [
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            pred_p_te_raw, pred_p_va, val_curve_p, rounds_p = fit_predict_recurrent_single_fixed(Xp_tr_df, Xp_te_df, y_p_tr, n_val, seed=run_seed, model_type="GRU", seq_len=12, max_epochs=50, batch_size=32, lower=0.05, upper=24.0)
            ...
            pred_k_te_raw, pred_k_va, val_curve_k, rounds_k = fit_predict_recurrent_single_fixed(Xk_tr_df, Xk_te_df, y_k_tr, n_val, seed=run_seed + 77, model_type="GRU", seq_len=12, max_epochs=50, batch_size=32, lower=0.0, upper=None)
            ...
            plot_one_curve(folder1, f"convergence_parking_time_window_{w}.png", val_curve_p,
                           f"Window={w} parking_time GRU Validation MAE")
            plot_one_curve(folder2, f"convergence_kWh_time_window_{w}.png", val_curve_k,
                           f"Window={w} kWhDelivered GRU Validation MAE")
    ...
    pd.DataFrame(metrics_all).to_csv(os.path.join(result_root, "metrics_gru_fixed_only_model.csv"), index=False)
    print("\nAll runs complete. Results saved to metrics_gru_fixed_only_model.csv")
