...
def main():
    ...
    def load_window_robust_data(window):
        ...
        dfw = dfw.dropna(subset=["connection_time_copy", "userID", "stationID"]).copy()

        tr0_fill = test_start - timedelta(days=window)
        train_mask_fill = (dfw["connection_time_copy"] >= tr0_fill) & (dfw["connection_time_copy"] < test_start)

        fill_records = []
        for col, valid_rule in [
            ("parking_time", "positive"),
            ("kWhDelivered", "nonnegative"),
        ]:
            train_values = pd.to_numeric(dfw.loc[train_mask_fill, col], errors="coerce").replace([np.inf, -np.inf], np.nan)
            if valid_rule == "positive":
                valid_train_values = train_values[train_values > 0]
            else:
                valid_train_values = train_values[train_values >= 0]

            if len(valid_train_values) == 0:
                all_values = pd.to_numeric(dfw[col], errors="coerce").replace([np.inf, -np.inf], np.nan)
                if valid_rule == "positive":
                    valid_all_values = all_values[all_values > 0]
                else:
                    valid_all_values = all_values[all_values >= 0]
                if len(valid_all_values) == 0:
                    raise ValueError(f"There is no valid data in {col} in {data_file} that can be used for median imputation.")
                fill_value = float(valid_all_values.median())
                fill_source = "global_valid_median"
            else:
                fill_value = float(valid_train_values.median())
                fill_source = "current_train_window_median"

            missing_train_before = int(dfw.loc[train_mask_fill, col].isna().sum())
            missing_total_before = int(dfw[col].isna().sum())
            dfw[col] = dfw[col].fillna(fill_value)

            fill_records.append({
                "column": col,
                "fill_value": fill_value,
                "fill_source": fill_source,
                "missing_train_before": missing_train_before,
                "missing_total_before": missing_total_before,
            })
        ...
        print(f"The current window uses the robust dataset: {data_file}")
        print(f"Missing value imputation method: Median imputation using valid data from the current window's training set.")
        for rec in fill_records:
            print(
                f"  {rec['column']}: fill={rec['fill_value']:.6f}, "
                f"source={rec['fill_source']}, "
                f"train_missing={rec['missing_train_before']}, "
                f"total_missing={rec['missing_total_before']}"
            )
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            final_k = np.maximum(inv_te_k, 0.05)
