...
def fit_predict_sarimax_fixed(X_train_df, X_test_df, y_train, n_val, lower=0.0, upper=None):
    try:
        from statsmodels.tsa.statespace.sarimax import SARIMAX
    except Exception as e:
        raise ImportError("This baseline needs statsmodels. Please run: pip install statsmodels") from e

    X_train = np.asarray(X_train_df.values, dtype=float)
    X_test = np.asarray(X_test_df.values, dtype=float)
    y_train = sanitize_target(y_train, lower=lower, upper=upper)

    X_fit_raw = X_train[:-n_val]
    X_val_raw = X_train[-n_val:]
    y_fit = y_train[:-n_val]
    y_val = y_train[-n_val:]
    ...
    try:
        ...
        future_exog = np.vstack([X_val, X_test_s])
        fcst = res.get_forecast(steps=len(future_exog), exog=future_exog).predicted_mean
        fcst = np.asarray(fcst, dtype=float)
        pred_val = fcst[:n_val]
        pred_test = fcst[n_val:]
    except Exception:
        fallback = np.median(y_fit) if len(y_fit) else lower
        pred_val = np.full(len(y_val), fallback, dtype=float)
        pred_test = np.full(len(X_test_s), fallback, dtype=float)

    pred_val = np.where(np.isfinite(pred_val), pred_val, np.median(y_fit) if len(y_fit) else lower)
    pred_test = np.where(np.isfinite(pred_test), pred_test, np.median(y_fit) if len(y_fit) else lower)
    ...
    val_mae = mae(y_val, pred_val) if len(y_val) else np.nan
    return pred_test, pred_val, [val_mae], 1
...
def main():
    ...
    def load_window_robust_data(window):
        ...
        print(f"The current window uses a robust dataset: {data_file}")
        print(f"Must be washed: {before} -> {len(dfw)}, delete {before - len(dfw)}")
    ...
    result_root = os.path.join(".", "results", f"{EXPERIMENT_NAME}_norway12_arima_fixed_only_model")
    base_dirs = [
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            pred_p_te_raw, pred_p_va, val_curve_p, rounds_p = fit_predict_sarimax_fixed(Xp_tr_df, Xp_te_df, y_p_tr, n_val, lower=0.05, upper=24.0)
            ...
            pred_k_te_raw, pred_k_va, val_curve_k, rounds_k = fit_predict_sarimax_fixed(Xk_tr_df, Xk_te_df, y_k_tr, n_val, lower=0.0, upper=None)
            ...
            plot_one_curve(folder1, f"convergence_parking_time_window_{w}.png", val_curve_p,
                           f"Window={w} parking_time ARIMA/SARIMAX Validation MAE")
            plot_one_curve(folder2, f"convergence_kWh_time_window_{w}.png", val_curve_k,
                           f"Window={w} kWhDelivered ARIMA/SARIMAX Validation MAE")
    ...
    pd.DataFrame(metrics_all).to_csv(os.path.join(result_root, "metrics_arima_fixed_only_model.csv"), index=False)
    print("\nAll runs complete. Results saved to metrics_arima_fixed_only_model.csv")
