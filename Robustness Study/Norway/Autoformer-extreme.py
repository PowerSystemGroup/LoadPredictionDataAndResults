...
def build_fixed_deep_model(model_type, seq_len, n_features):
    from tensorflow.keras import layers, models

    mt = model_type.upper()
    inp = layers.Input(shape=(seq_len, n_features), name="feature_sequence")

    if mt == "TFT":
        gate = layers.Dense(n_features, activation="sigmoid", name="variable_selection_gate")(inp)
        x = layers.Multiply(name="gated_inputs")([inp, gate])
        x = layers.Dense(32, activation="elu", name="feature_projection")(x)
        enc = layers.LSTM(32, return_sequences=True, name="temporal_lstm_encoder")(x)
        attn = layers.MultiHeadAttention(num_heads=2, key_dim=16, name="temporal_attention")(enc, enc)
        x = layers.Add(name="attention_residual")([enc, attn])
        x = layers.LayerNormalization(name="post_attention_norm")(x)
        x = layers.GlobalAveragePooling1D(name="temporal_pooling")(x)
        x = layers.Dense(16, activation="relu", name="dense_head")(x)

    elif mt == "AUTOFORMER":
        trend = layers.AveragePooling1D(pool_size=3, strides=1, padding="same", name="moving_average_trend")(inp)
        seasonal = layers.Subtract(name="seasonal_component")([inp, trend])
        seasonal = layers.Dense(32, activation="linear", name="seasonal_projection")(seasonal)
        trend = layers.Dense(32, activation="linear", name="trend_projection")(trend)
        attn = layers.MultiHeadAttention(num_heads=2, key_dim=16, name="auto_correlation_attention")(seasonal, seasonal)
        x = layers.Add(name="decomposition_fusion")([trend, attn])
        x = layers.LayerNormalization(name="fusion_norm")(x)
        x = layers.GlobalAveragePooling1D(name="temporal_pooling")(x)
        x = layers.Dense(16, activation="relu", name="dense_head")(x)

    elif mt == "PATCHTST":
        x = layers.Conv1D(32, kernel_size=4, strides=4, padding="same", name="patch_embedding")(inp)
        attn = layers.MultiHeadAttention(num_heads=2, key_dim=16, name="patch_attention")(x, x)
        x = layers.Add(name="patch_residual")([x, attn])
        x = layers.LayerNormalization(name="patch_norm")(x)
        ff = layers.Dense(32, activation="relu", name="patch_ffn_1")(x)
        ff = layers.Dense(32, activation="linear", name="patch_ffn_2")(ff)
        ...
        x = layers.Concatenate(name="linear_decomposition_concat")([trend_flat, seasonal_flat])

    elif mt == "ITRANSFORMER":
        x = layers.Permute((2, 1), name="inverted_tokens")(inp)
        x = layers.Dense(32, activation="linear", name="feature_token_embedding")(x)
        attn = layers.MultiHeadAttention(num_heads=2, key_dim=16, name="feature_attention")(x, x)
        x = layers.Add(name="feature_attention_residual")([x, attn])
        x = layers.LayerNormalization(name="feature_attention_norm")(x)
        ff = layers.Dense(32, activation="relu", name="feature_ffn_1")(x)
        ff = layers.Dense(32, activation="linear", name="feature_ffn_2")(ff)
        x = layers.Add(name="feature_ffn_residual")([x, ff])
        x = layers.LayerNormalization(name="feature_ffn_norm")(x)
        x = layers.GlobalAveragePooling1D(name="feature_pooling")(x)
        x = layers.Dense(16, activation="relu", name="dense_head")(x)
    ...
    return models.Model(inp, out, name=f"{mt}_Fixed_Only_Model")


def fit_predict_deep_single_fixed(X_train_df, X_test_df, y_train, n_val, seed, model_type="TFT",
    ...
    model = build_fixed_deep_model(model_type, seq_len, X_seq.shape[-1])
...
def main():
    ...
    EXPERIMENT_NAME = f"norway_{ROBUST_TYPE}_{ROBUST_RATIO}_keepLOF_woMedian"
    ...
    def load_window_robust_data(window):
        ...
        if missing_cols:
            raise ValueError(f"{data_file} Required fields are missing: {missing_cols}")
        ...
        dfw = dfw.dropna(subset=required_cols).copy()
    ...
    result_root = os.path.join(".", "results", f"{EXPERIMENT_NAME}_norway12_autoformer_fixed_only_model")
    base_dirs = [
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            pred_p_te_raw, pred_p_va, val_curve_p, rounds_p = fit_predict_deep_single_fixed(Xp_tr_df, Xp_te_df, y_p_tr, n_val, seed=run_seed, model_type="AUTOFORMER", seq_len=24, max_epochs=50, batch_size=32, lower=0.05, upper=24.0)
            ...
            pred_k_te_raw, pred_k_va, val_curve_k, rounds_k = fit_predict_deep_single_fixed(Xk_tr_df, Xk_te_df, y_k_tr, n_val, seed=run_seed + 77, model_type="AUTOFORMER", seq_len=24, max_epochs=50, batch_size=32, lower=0.0, upper=None)
            ...
            plot_one_curve(folder1, f"convergence_parking_time_window_{w}.png", val_curve_p,
                           f"Window={w} parking_time Autoformer Validation MAE")
            plot_one_curve(folder2, f"convergence_kWh_time_window_{w}.png", val_curve_k,
                           f"Window={w} kWhDelivered Autoformer Validation MAE")
    ...
    pd.DataFrame(metrics_all).to_csv(os.path.join(result_root, "metrics_autoformer_fixed_only_model.csv"), index=False)
    print("\nAll runs complete. Results saved to metrics_autoformer_fixed_only_model.csv")
