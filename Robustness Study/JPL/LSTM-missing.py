...
def main():
    ...
    EXPERIMENT_NAME = os.path.splitext(os.path.basename(DATA_FILE))[0] + "_trainMedian_LSTM_keepLOF_woMedian"
    ...
    if target_missing.sum() > 0:
        raise ValueError(
            ...
            "Please only impute missing values in the input features; do not delete `parking_time` or `kWhDelivered`."
        )
        ...
        for w in windows:
            ...
            for col, info in fill_summary.items():
                ...
                imputation_records.append({
                    ...
                    "train_missing_before": info["train_missing_before"],
                    "test_missing_before": info["test_missing_before"],
                    "train_missing_after": info["train_missing_after"],
                    "test_missing_after": info["test_missing_after"],
                })
            ...
            assert_no_nan_matrix(tr, sim_feats, "Similar conversation training input")
            assert_no_nan_matrix(te, sim_feats, "Sample test input")
            ...
            Xtr_tensor = torch.tensor(Xtr_p, dtype=torch.float32).unsqueeze(1)  # shape: (N, 1, D)
