...
class LearnablePositionalEncoding(nn.Module):
    def __init__(self, max_tokens: int, d_model: int):
        super().__init__()
...
class iTransformerOneStep(nn.Module):
    def __init__(
        self,
        input_dim: int,
        seq_len: int,
        ...
        self.token_projection = nn.Linear(seq_len, d_model)
        self.var_pos = LearnablePositionalEncoding(max_tokens=input_dim, d_model=d_model)
        ...
        self.head = nn.Sequential(
            nn.Linear(input_dim * d_model, hidden_head),
            nn.GELU(),
            nn.Dropout(dropout),
...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"There are not enough historical samples; currently, there are only {len(hist)} entries, which is insufficient to construct an input sequence of length {encoder_length}.")
...
def fit_itransformer_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct the iTransformer sequence: {len(train_data)}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty, so iTransformer cannot be trained.")
    ...
    for epoch in range(max_epochs):
        ...
        for xb, yb, wb in train_loader:
            ...
            if not loss.requires_grad:
                raise RuntimeError(
                    "No gradient computation graph for training loss: Please check if torch.no_grad() or torch.inference_mode() is used in the outer layer,"
                    "or if the requires_grad attribute of the model parameter is set to false."
                )
...
def recursive_forecast_itransformer(
    model: nn.Module,
...
def main():
    ...
    for run in range(1, n_runs + 1):
        ...
        for base in base_dirs:
            ...
            print(f"The folder has been verified to exist: {folder_path}")
        ...
        for w in windows:
            ...
            try:
                ...
                pred_p_te = recursive_forecast_itransformer(
                    model=pt_fit["model"],
                    history_df=hist_pt,
                    future_df=future_pt,
                    feature_cols=pf,
                ...
                print(f"[ITRANSFORMER ROBUST ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                pred_te_bc = recursive_forecast_itransformer(
                    model=k_fit["model"],
                    history_df=hist_k,
                    future_df=future_k,
                    feature_cols=feats_k,
                ...
                print(f"[ITRANSFORMER ROBUST ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
