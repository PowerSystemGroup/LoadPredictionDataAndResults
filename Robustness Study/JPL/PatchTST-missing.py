...
class PatchTSTOneStep(nn.Module):
    def __init__(
        ...
        self.num_patches = 1 + max(0, (seq_len - self.patch_len) // self.stride)
        ...
        self.head = nn.Sequential(
            nn.Linear(input_dim * d_model, 2 * d_model),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, x):
        ...
        if L < self.patch_len:
            ...
            x = torch.cat([x[:, :, :1].repeat(1, 1, pad_len), x], dim=-1)
            L = x.shape[-1]

        patches = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)
        B, F, N, P = patches.shape
        patches = self.patch_proj(patches)
        patches = patches + self.pos_embed[:, :N, :].unsqueeze(1)
        patches = self.dropout(patches)

        patches = patches.reshape(B * F, N, -1)
        tokens = self.encoder(patches)
        tokens = self.norm(tokens)
        channel_repr = tokens.mean(dim=1).reshape(B, F, -1)
        out = self.head(channel_repr.reshape(B, -1)).squeeze(-1)
        return out
...
def fit_patchtst_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct a PatchTST sequence: {len(train_data)}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; PatchTST cannot be trained.")

    seq_len = X_train.shape[1]
    actual_patch_len = max(2, min(patch_len, seq_len))
    actual_stride = max(1, min(stride, actual_patch_len))
...
def recursive_forecast_patchtst(
    model: nn.Module,
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            try:
                ...
                pred_p_te = recursive_forecast_patchtst(
                    model=pt_fit["model"],
                    history_df=hist_pt,
                    future_df=future_pt,
                    feature_cols=pf,
                ...
                print(f"[PATCHTST ROBUST ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                pred_te_bc = recursive_forecast_patchtst(
                    model=k_fit["model"],
                    history_df=hist_k,
                    future_df=future_k,
                    feature_cols=feats_k,
                ...
                print(f"[PATCHTST ROBUST ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
