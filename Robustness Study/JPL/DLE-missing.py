...
def fill_missing_by_train_median(tr, te, cols, global_fallback=None):
    ...
    for col in cols:
        ...
        tr_missing_before = int(tr[col].isna().sum())
        te_missing_before = int(te[col].isna().sum())

        fill_value = tr[col].median(skipna=True)
        if pd.isna(fill_value):
            fill_value = global_fallback.get(col, np.nan)
        if pd.isna(fill_value):
            raise ValueError(
                f"Column {col} is entirely missing within the current training window, and historical default values are not available, so median imputation cannot be performed."
            )
        ...
        summary[col] = {
            ...
            "train_missing_before": tr_missing_before,
            "test_missing_before": te_missing_before,
            "train_missing_after": int(tr[col].isna().sum()),
            "test_missing_after": int(te[col].isna().sum())
        }

    return tr, te, summary


def assert_no_nan_matrix(df, cols, stage_name):
    miss = df[cols].isna().sum()
    miss = miss[miss > 0]
    if len(miss) > 0:
        raise ValueError(f"There are still missing values in {stage_name}: {miss.to_dict()}")
...
def main():
    ...
    EXPERIMENT_NAME = os.path.splitext(os.path.basename(DATA_FILE))[0] + "_trainMedian_keepLOF_woMedian"
    ...
    target_cols = ["parking_time", "kWhDelivered"]
    target_missing = df0[target_cols].isna().sum()
    if target_missing.sum() > 0:
        raise ValueError(
            f"There are missing values in the target column, so a robustness assessment for missing values cannot be performed directly: {target_missing.to_dict()}."
            "Please only construct missing values for the input features; do not delete parking_time / kWhDelivered."
        )
    ...
    hist_before_test = df0[df0.connection_time_copy < test_start]
    GLOBAL_TRAIN_FALLBACK = {
        col: hist_before_test[col].median(skipna=True)
        for col in MISSING_INPUT_COLS
        if col in hist_before_test.columns
    }
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            tr, te, fill_summary = fill_missing_by_train_median(
                tr, te, MISSING_INPUT_COLS, global_fallback=GLOBAL_TRAIN_FALLBACK
            )

            print(f"  Missing input before fill: {raw_missing_info}")
            for col, info in fill_summary.items():
                print(
                    f"    {col}: train_missing={info['train_missing_before']} -> {info['train_missing_after']}, "
                    f"test_missing={info['test_missing_before']} -> {info['test_missing_after']}, "
                    f"fill_value={info['fill_value']:.6f}"
                )
            ...
            assert_no_nan_matrix(tr, sim_feats, f"Run={run}, Window={w}, Similar conversation features in the training set")
            assert_no_nan_matrix(te, sim_feats, f"Run={run}, Window={w}, Similar session features in the test set")
            ...
            assert_no_nan_matrix(tr, pf, f"Run={run}, Window={w}, parking_time training input")
            assert_no_nan_matrix(te, pf, f"Run={run}, Window={w}, parking_time test input")
            ...
            assert_no_nan_matrix(tr, feats_k, f"Run={run}, Window={w}, kWhDelivered training input")
            assert_no_nan_matrix(te, feats_k, f"Run={run}, Window={w}, kWhDelivered Test Input")
    ...
    imputation_path = os.path.join(".", "results", EXPERIMENT_NAME, "missing_imputation_summary.csv")
    pd.DataFrame(imputation_records).to_csv(imputation_path, index=False)

    print(f"\nAll runs complete. Results saved to {metrics_path}")
    print(f"Missing imputation summary saved to {imputation_path}")
