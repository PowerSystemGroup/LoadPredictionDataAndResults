...
def fill_missing_by_train_median(tr, te, cols, global_fallback=None):
    ...
    for col in cols:
        ...
        if pd.isna(fill_value):
            raise ValueError(
                f"Column {col} is entirely missing within the current training window, and historical default values are unavailable,"
                f"so median imputation cannot be performed."
            )
...
def assert_no_nan_matrix(df, cols, stage_name):
    ...
    miss = miss[miss > 0]
    if len(miss) > 0:
        raise ValueError(f"{stage_name} There are still missing values in the data: {miss.to_dict()}")
...
def main():
    ...
    EXPERIMENT_NAME = os.path.splitext(os.path.basename(DATA_FILE))[0] + "_trainMedian_ARIMA_keepLOF_woMedian"
