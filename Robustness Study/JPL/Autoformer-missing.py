...
MISSING_INPUT_COLS = ["kWhRequested", "Requested_parking_time"]


def fill_missing_inputs_by_train_median(
    tr: pd.DataFrame,
    te: pd.DataFrame,
    input_cols=None,
    verbose: bool = True,
    ...
    for col in input_cols:
        if col in te.columns:
            te_numeric = pd.to_numeric(te[col], errors="coerce")
            n_miss = int(te_numeric.isna().sum())
            if n_miss > 0:
                missing_before[col] = n_miss

    if verbose:
        print(f"    Missing input before fill: {missing_before}")
    ...
    for col in input_cols:
        ...
        tr_before = int(tr[col].isna().sum())
        te_before = int(te[col].isna().sum())

        fill_value = float(tr[col].median())
        if not np.isfinite(fill_value):
            both = pd.concat([tr[col], te[col]], axis=0)
            fill_value = float(both.median())
        if not np.isfinite(fill_value):
            fill_value = 0.0

        tr[col] = tr[col].fillna(fill_value)
        te[col] = te[col].fillna(fill_value)

        tr_after = int(tr[col].isna().sum())
        te_after = int(te[col].isna().sum())

        if verbose:
            print(
                f"    {col}: train_missing={tr_before} -> {tr_after}, "
                f"test_missing={te_before} -> {te_after}, fill_value={fill_value:.6f}"
            )
...
class AutoCorrelation(nn.Module):
    def __init__(self, d_model: int, n_heads: int = 4, factor: float = 2.0, dropout: float = 0.1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        q = self.q_proj(x).view(B, L, H, E).permute(0, 2, 3, 1)
        k = self.k_proj(x).view(B, L, H, E).permute(0, 2, 3, 1)
        v = self.v_proj(x).view(B, L, H, E).permute(0, 2, 1, 3)

        q_fft = torch.fft.rfft(q, dim=-1)
        k_fft = torch.fft.rfft(k, dim=-1)
        corr = torch.fft.irfft(q_fft * torch.conj(k_fft), n=L, dim=-1)
        corr = corr.mean(dim=2)

        top_k = max(1, int(self.factor * math.log(L + 1)))
        top_k = min(top_k, L)
        weights, delays = torch.topk(corr, k=top_k, dim=-1)
        attn = torch.softmax(weights / math.sqrt(E), dim=-1)
        attn = self.dropout(attn)

        out = torch.zeros_like(v)
        base_idx = torch.arange(L, device=x.device).view(1, 1, L).expand(B, H, L)

        for i in range(top_k):
            delay = delays[:, :, i]
            gather_idx = (base_idx - delay.unsqueeze(-1)) % L
            gather_idx = gather_idx.unsqueeze(-1).expand(B, H, L, E)
            shifted = torch.gather(v, 2, gather_idx)
            out = out + attn[:, :, i].unsqueeze(-1).unsqueeze(-1) * shifted
...
class AutoformerBlock(nn.Module):
    def __init__(self, d_model: int, n_heads: int = 4, d_ff: int = 128,
                 moving_avg: int = 25, factor: float = 2.0, dropout: float = 0.1):
        super().__init__()
        self.auto_corr = AutoCorrelation(d_model=d_model, n_heads=n_heads, factor=factor, dropout=dropout)
...
class AutoformerOneStep(nn.Module):
    def __init__(self, input_dim: int, d_model: int = 64, n_heads: int = 4,
                 e_layers: int = 2, d_ff: int = 128, moving_avg: int = 25,
                 factor: float = 2.0, dropout: float = 0.1):
        super().__init__()
...
class SequenceForecastDataset(Dataset):
    def __init__(self, sequences: np.ndarray, targets: np.ndarray, weights: np.ndarray = None):
...
def build_supervised_samples(
    ...
    for t in index_list:
        ...
        if sample_weights is None:
            ...
            weights.append(float(sample_weights[t]))

    if len(sequences) == 0:
        return (
            np.empty((0, encoder_length + 1, len(feature_cols) + 1), dtype=np.float32),
...
def fit_autoformer_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct an Autoformer sequence: {len(train_data)}")
    ...
    X_train, y_train, w_train = build_supervised_samples(
        df_model=df_model,
    ...
    X_val, y_val, _ = build_supervised_samples(
        df_model=df_model,
        feature_cols=feature_cols,
        target_col=target_col,
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; the Autoformer cannot be trained.")

    train_loader = DataLoader(SequenceForecastDataset(X_train, y_train, w_train), batch_size=batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(SequenceForecastDataset(X_val, y_val), batch_size=batch_size, shuffle=False, num_workers=0)

    model = AutoformerOneStep(
...
def recursive_forecast_autoformer(
    model: nn.Module,
...
def make_finalize_kwh_row(pt_transform, eps: float):
    def _finalize(hist_df: pd.DataFrame, row: pd.Series, pred_bc: float):
        row = row.copy()
        pred_bc_safe = float(max(pred_bc, 1e-6))
        pred_kwh = float(pt_transform.inverse_transform(np.array([[pred_bc_safe]], dtype=float)).reshape(-1)[0] - eps)
        pred_kwh = max(pred_kwh, 0.0)
        row["kWhDelivered_bc"] = float(pred_bc_safe)
        row["target_kwh_bc"] = float(pred_bc_safe)
...
def main():
    ...
    DATA_FILE = "jpl_missing_5.csv"
    MODEL_NAME = "Autoformer"
    EXPERIMENT_NAME = os.path.splitext(os.path.basename(DATA_FILE))[0] + "_missingMedian_" + MODEL_NAME
    ...
    for run in range(1, n_runs + 1):
        ...
        if "kWhRequested" in hist_all.columns:
            hist_all["kWhRequested"] = pd.to_numeric(hist_all["kWhRequested"], errors="coerce")
            _cluster_fill = float(hist_all["kWhRequested"].median())
            if not np.isfinite(_cluster_fill):
                _cluster_fill = 0.0
            hist_all["kWhRequested"] = hist_all["kWhRequested"].fillna(_cluster_fill)
        ...
        for w in windows:
            ...
            tr, te = fill_missing_inputs_by_train_median(tr, te, MISSING_INPUT_COLS, verbose=True)

            tr, te = denoise_train_test(tr, te)

            tr["kWhDelivered"] = pd.to_numeric(tr["kWhDelivered"], errors="coerce").clip(lower=0.0)
            te["kWhDelivered"] = pd.to_numeric(te["kWhDelivered"], errors="coerce").clip(lower=0.0)

            tr["age_days"] = (test_start - tr.connection_time_copy).dt.days
            w_decay = np.exp(-0.015 * tr["age_days"]).astype(np.float32).values
            ...
            sample_weight_pt = np.concatenate([w_decay, np.ones(len(te_pt), dtype=np.float32)])
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    f"Window={w}  parking_time  Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[AUTOFORMER ROBUST ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
                mae_p, sm_p, rounds_p, time_p_ms = np.nan, np.nan, 0, 0.0

            pt_transform = PowerTransformer(method="box-cox", standardize=False)
            y_all_k = np.maximum(tr.kWhDelivered.values, 0.0) + eps
            ...
            te_k["rk3"] = med_k
            prev_vals = []
            for i in range(len(te_k)):
                if i >= 1:
                    te_k.at[i, "pk1"] = prev_vals[i - 1]
                if i >= 2:
                    te_k.at[i, "pk2"] = prev_vals[i - 2]
                if i >= 3:
                    te_k.at[i, "rk3"] = np.mean(prev_vals[i - 3:i])
                prev_vals.append(te_k.at[i, "kWhDelivered"])
            ...
            all_k["kWhDelivered_bc"] = pt_transform.transform((np.maximum(all_k["kWhDelivered"].values, 0.0) + eps).reshape(-1, 1)).flatten()
            ...
            sample_weight_k = np.concatenate([w_decay, np.ones(len(te_k), dtype=np.float32)])
            ...
            try:
                ...
                clipped = np.maximum(pred_te_bc, 1e-6)
                inv_te_k = pt_transform.inverse_transform(clipped).flatten() - eps
                final_k = np.maximum(inv_te_k, 0.0).copy()
                ...
                save_curve_from_list(
                    ...
                    f"Window={w}  kWhDelivered  Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[AUTOFORMER ROBUST ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
