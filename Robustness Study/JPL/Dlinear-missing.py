...
def denoise_train_test(tr: pd.DataFrame, te: pd.DataFrame):
    ...
    for col in ["kWhDelivered"]:
        ...
        coeffs[1:] = [pywt.threshold(c, last_thr, "soft") for c in coeffs[1:]]
...
class DLinearOneStep(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (seasonal_out + trend_out).squeeze(-1)
...
def fit_dlinear_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct the DLinear sequence: {len(train_data)}")
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            try:
                ...
                print(f"[DLINEAR ROBUST ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                print(f"[DLINEAR ROBUST ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
