...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"There is insufficient historical data; currently, there is only {len(hist)}, "
                         f"Unable to construct an input sequence of length {encoder_length}")
...
def main():
    ...
    EXPERIMENT_NAME = os.path.splitext(os.path.basename(DATA_FILE))[0] + "_keepLOF_woMedian_" + MODEL_NAME
