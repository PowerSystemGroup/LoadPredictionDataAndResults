...
class PatchTSTOneStep(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # [B, F, N, P]
        B, F, N, P = patches.shape
        patches = self.patch_proj(patches)  # [B, F, N, D]
        ...
        channel_repr = tokens.mean(dim=1).reshape(B, F, -1)  # [B, F, D]
...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"There are insufficient historical samples; currently, there are only {len(hist)} entries, "
...
def fit_patchtst_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct an PatchTST sequence: {len(train_data)}")
...
def main():
    ...
    os.makedirs("./Caltech/results/PatchTST_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "Caltech", "results", "PatchTST_strict", "parking_time"),
        os.path.join(".", "Caltech", "results", "PatchTST_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "Caltech", "results", "PatchTST_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
                    f"Window={w}  parking_time (PatchTST strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[PATCHTST STRICT ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "Caltech", "results", "PatchTST_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
                    f"Window={w}  kWhDelivered (PatchTST strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[PATCHTST STRICT ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
    ...
    pd.DataFrame(metrics_all).to_csv("./Caltech/results/PatchTST_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./Caltech/results/PatchTST_strict/metrics_mixed_dynamic.csv")
