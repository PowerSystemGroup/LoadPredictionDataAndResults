...
def fit_itransformer_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"There are too few training samples to construct an iTransformer sequence: {len(train_data)}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; iTransformer cannot be trained.")
...
def main():
    ...
    os.makedirs("./Caltech/results/iTransformer_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "Caltech", "results", "iTransformer_strict", "parking_time"),
        os.path.join(".", "Caltech", "results", "iTransformer_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "Caltech", "results", "iTransformer_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
                    f"Window={w}  parking_time (iTransformer strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[ITRANSFORMER STRICT ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "Caltech", "results", "iTransformer_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
                    f"Window={w}  kWhDelivered (iTransformer strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[ITRANSFORMER STRICT ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
    ...
    pd.DataFrame(metrics_all).to_csv("./Caltech/results/iTransformer_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./Caltech/results/iTransformer_strict/metrics_mixed_dynamic.csv")
