...
def main():
    ...
    base_dirs = [
        os.path.join(".", "results", "caltech", "parking_time"),
        os.path.join(".", "results", "caltech", "kWhDelivered"),
    ]
    ...
    for run in range(1, n_runs + 1):
        ...
        for base in base_dirs:
            ...
            print(f"It has been confirmed that the folder exists: {folder_path}")
        print(f"\n=== Run {run} ===")
        df = df0.copy()
        ...
        for w in windows:
            ...
            plt.tight_layout()
            folder1 = os.path.join(".", "results", "caltech", "parking_time", str(run))
            ...
            ax.set_ylabel("Validation MAE")
            plt.tight_layout()
            folder2 = os.path.join(".", "results", "caltech", "kWhDelivered", str(run))
    ...
    pd.DataFrame(metrics_all).to_csv("./results/caltech/metrics_mixed_dynamic.csv", index=False)
