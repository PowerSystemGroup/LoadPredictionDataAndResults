...
def find_input_file():
    ...
    raise FileNotFoundError("No found Charging_Data_JX_clustered_cleaned_final.csv")
...
def build_station_quality_table(df):
    ...
    if tab.empty:
        raise RuntimeError("Unable to build the site quality table. Please check if there is historical data prior to the test date.")
...
def apply_quality_filter(df, station_quality):
    ...
    if USE_STATION_SELECTION and cfg["top_stations"] is not None:
        ...
        if candidates.empty:
            print("Warning: No sites meet the minimum sample size requirement. Returning without performing site filtering.")
        else:
...
def filter_clusters_before_modeling(df):
    ...
    for cid in sorted(df["cluster_id"].dropna().astype(int).unique().tolist()):
        ...
        if test_count == 0:
            reasons.append("No data in the test interval")
...
def main():
    ...
    if len(retained_cluster_ids) == 0:
        raise RuntimeError("No categories meet the filter criteria; modeling is not possible. Please relax the station/top/quantile filters.")
