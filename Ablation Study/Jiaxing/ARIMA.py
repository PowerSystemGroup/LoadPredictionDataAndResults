...
def find_input_file():
    ...
    raise FileNotFoundError("The file Charging_Data_JX_clustered_cleaned_final.csv was not found. "
                            "Please place it in the current directory, /kaggle/input, or /mnt/data.")
...
def load_raw_data():
    ...
    if time_col not in df.columns:
        raise ValueError("The input file is missing the start_time or connection_time_copy time columns.")
    ...
    if "parking_time" not in df.columns:
        if end_col not in df.columns:
            raise ValueError("The input file is missing `parking_time`, and there is no `end_time` to calculate the parking duration.")
        df["parking_time"] = (df[end_col] - df["connection_time_copy"]).dt.total_seconds() / 3600.0

    if "kWhDelivered" not in df.columns:
        raise ValueError("The input file is missing the kWhDelivered column.")

    if "cluster_id" not in df.columns:
        raise ValueError("The input file is missing the `cluster_id` column, so it cannot be modeled by region/category.")
    ...
    print(f"Basic data cleaning: {before} -> {len(df)}; delete {before - len(df)} rows.")
...
def apply_quality_filter(df, station_quality):
    ...
    if USE_STATION_SELECTION and cfg["top_stations"] is not None:
        ...
        if candidates.empty:
            print("Warning: No sites meet the minimum sample size requirement; the process is terminated without site screening.")
        else:
...
if torch is not None:
    ...
    class PatchTSTMiniRegressor(nn.Module):
        ...
        def forward(self, x):
            ...
            patches = x.unfold(dimension=1, size=self.patch_len, step=self.stride)  # [B, P, F, patch]
...
def main():
    ...
    if not pred_df.empty:
        print(f"Prediction results for each sample: {OUTPUT_PRED_CSV}")
