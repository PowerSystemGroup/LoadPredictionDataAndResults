...
if os.path.exists("/kaggle/working"):
    OUTPUT_DIR = "/kaggle/working/jx_dle_complexity_results"
else:
    OUTPUT_DIR = "./jx_dle_complexity_results"
...
OUTPUT_COST_CSV = os.path.join(OUTPUT_DIR, "jx_computational_cost_summary.csv")
OUTPUT_COMPLEXITY_DETAIL_CSV = os.path.join(OUTPUT_DIR, "jx_complexity_detail_by_group.csv")
OUTPUT_COMPLEXITY_SUMMARY_CSV = os.path.join(OUTPUT_DIR, "jx_complexity_summary_by_window.csv")
OUTPUT_COMPLEXITY_TABLE_CSV = os.path.join(OUTPUT_DIR, "jx_complexity_table_for_paper.csv")
OUTPUT_COMPLEXITY_XLSX = os.path.join(OUTPUT_DIR, "jx_complexity_analysis.xlsx")
...
class MemoryMonitor:
    def __init__(self, interval=0.05):
        ...
        self.running = False
        self.thread = None
        self.start_rss_mb = np.nan
        self.end_rss_mb = np.nan
        self.peak_rss_mb = np.nan
        self.peak_delta_mb = np.nan

    def start(self):
        gc.collect()
        self.start_rss_mb = get_process_memory_mb()
    ...
    def _monitor(self):
        while self.running:
            ...
            if not np.isnan(mem_now):
                self.peak_rss_mb = max(self.peak_rss_mb, mem_now)
    ...
    def stop(self):
        ...
        self.end_rss_mb = get_process_memory_mb()
        if np.isnan(self.peak_rss_mb) and not np.isnan(self.end_rss_mb):
            self.peak_rss_mb = self.end_rss_mb

        self.peak_delta_mb = self.peak_rss_mb - self.start_rss_mb \
            if not np.isnan(self.peak_rss_mb) and not np.isnan(self.start_rss_mb) else np.nan
        gc.collect()


def backend_warmup():
    X_dummy = np.random.rand(200, 10)
    y_dummy = np.random.rand(200)
    ...
    model_dummy = xgb.train(
        params_dummy,
        dtrain_dummy,
        num_boost_round=3,
        verbose_eval=False
    )
    _ = model_dummy.predict(dtrain_dummy)

    lgb_dummy = lgb.LGBMRegressor(
        objective="regression",
        n_estimators=5,
        learning_rate=0.05,
        verbosity=-1
    )
    lgb_dummy.fit(X_dummy, y_dummy)
    _ = lgb_dummy.predict(X_dummy)

    del X_dummy, y_dummy, dtrain_dummy, model_dummy, lgb_dummy
    gc.collect()
...
def find_input_file():
    ...
    raise FileNotFoundError(
        "Charging_Data_JX_clustered_cleaned_final.csv was not found. Please place the file in the current directory, /kaggle/input, or /mnt/data."
    )
...
def fit_one_cluster(cluster_df, window, run):
    ...
    mem_monitor_p = MemoryMonitor(interval=0.05)
    mem_monitor_p.start()
    ...
    mem_monitor_p.stop()
    peak_mem_parking_mb = mem_monitor_p.peak_delta_mb
    peak_rss_parking_mb = mem_monitor_p.peak_rss_mb

    mem_monitor_k = MemoryMonitor(interval=0.05)
    mem_monitor_k.start()
    ...
    mem_monitor_k.stop()
    peak_mem_kWh_mb = mem_monitor_k.peak_delta_mb
    peak_rss_kWh_mb = mem_monitor_k.peak_rss_mb
    ...
    result_row = {
        ...
        "parking_train_test_time_ms": time_parking_sec * 1000.0,
        "parking_peak_memory_MB": peak_mem_parking_mb,
        "charging_train_test_time_ms": time_kwh_sec * 1000.0,
        "charging_peak_memory_MB": peak_mem_kWh_mb,

        "rounds_pt": len(pt_val_maes),
        "rounds_kWh": len(kwh_val_maes),
        "time_pt_ms": time_parking_sec * 1000.0,
        "time_kWh_ms": time_kwh_sec * 1000.0,
        "window_time_ms": time_total_sec * 1000.0,
        "peak_mem_pt_mb": peak_mem_parking_mb,
        "peak_rss_pt_mb": peak_rss_parking_mb,
        "peak_mem_kWh_mb": peak_mem_kWh_mb,
        "peak_rss_kWh_mb": peak_rss_kWh_mb,

        "peak_mem_parking_mb": peak_mem_parking_mb,
        "peak_rss_parking_mb": peak_rss_parking_mb,
    }
...
def save_results(overall_df, cluster_detail_df, pred_df, scenario_map, filter_summary, station_quality):
    ...
    cost_cols = [
        "run", "window", "clusters_used", "test_samples",
        "sum_time_parking_sec", "sum_time_kWh_sec", "sum_time_total_sec",
        "time_pt_mean_ms", "time_kWh_mean_ms", "window_time_mean_ms",
        "peak_mem_pt_mean_mb", "peak_mem_pt_max_mb",
        "peak_mem_kWh_mean_mb", "peak_mem_kWh_max_mb",
        "mean_peak_mem_parking_mb", "max_peak_mem_parking_mb",
        "mean_peak_mem_kWh_mb", "max_peak_mem_kWh_mb"
    ]
    cost_summary = overall_df[[c for c in cost_cols if c in overall_df.columns]].copy() if not overall_df.empty else pd.DataFrame()
    if not cost_summary.empty:
        cost_summary.to_csv(OUTPUT_COST_CSV, index=False, encoding="utf-8-sig")

    if cluster_detail_df.empty:
        complexity_detail = pd.DataFrame()
        complexity_summary = pd.DataFrame()
        paper_table = pd.DataFrame()
    else:
        needed_cols = [
            "run", "window", "cluster_id", "scenario_name",
            "n_train", "n_test",
            "time_pt_ms", "peak_mem_pt_mb", "peak_rss_pt_mb",
            "time_kWh_ms", "peak_mem_kWh_mb", "peak_rss_kWh_mb",
            "window_time_ms", "mae_parking", "smape_parking", "mae_kWh", "smape_kWh"
        ]
        existing_cols = [c for c in needed_cols if c in cluster_detail_df.columns]
        complexity_detail = cluster_detail_df[existing_cols].copy()
        rename_map = {
            "cluster_id": "group_id",
            "scenario_name": "group_name",
            "window": "training_window_days",
            "n_train": "train_samples",
            "n_test": "test_samples",
            "time_pt_ms": "parking_duration_time_ms",
            "peak_mem_pt_mb": "parking_duration_peak_memory_MB",
            "peak_rss_pt_mb": "parking_duration_peak_RSS_MB",
            "time_kWh_ms": "charging_capacity_time_ms",
            "peak_mem_kWh_mb": "charging_capacity_peak_memory_MB",
            "peak_rss_kWh_mb": "charging_capacity_peak_RSS_MB",
            "window_time_ms": "total_group_window_time_ms",
        }
        complexity_detail = complexity_detail.rename(columns=rename_map)

        if "parking_duration_time_ms" in complexity_detail.columns:
            complexity_detail["parking_duration_time_sec"] = complexity_detail["parking_duration_time_ms"] / 1000.0
        if "charging_capacity_time_ms" in complexity_detail.columns:
            complexity_detail["charging_capacity_time_sec"] = complexity_detail["charging_capacity_time_ms"] / 1000.0
        if "total_group_window_time_ms" in complexity_detail.columns:
            complexity_detail["total_group_window_time_sec"] = complexity_detail["total_group_window_time_ms"] / 1000.0

        g = cluster_detail_df.groupby(["run", "window"])
        complexity_summary = g.agg(
            groups_used=("cluster_id", "nunique"),
            train_samples_sum=("n_train", "sum"),
            train_samples_mean=("n_train", "mean"),
            test_samples_sum=("n_test", "sum"),
            test_samples_mean=("n_test", "mean"),
            parking_duration_time_sum_ms=("time_pt_ms", "sum"),
            parking_duration_time_mean_ms=("time_pt_ms", "mean"),
            parking_duration_time_max_ms=("time_pt_ms", "max"),
            parking_duration_peak_memory_mean_MB=("peak_mem_pt_mb", "mean"),
            parking_duration_peak_memory_max_MB=("peak_mem_pt_mb", "max"),
            charging_capacity_time_sum_ms=("time_kWh_ms", "sum"),
            charging_capacity_time_mean_ms=("time_kWh_ms", "mean"),
            charging_capacity_time_max_ms=("time_kWh_ms", "max"),
            charging_capacity_peak_memory_mean_MB=("peak_mem_kWh_mb", "mean"),
            charging_capacity_peak_memory_max_MB=("peak_mem_kWh_mb", "max"),
            total_window_time_sum_ms=("window_time_ms", "sum"),
            total_window_time_mean_ms=("window_time_ms", "mean"),
        ).reset_index().rename(columns={"window": "training_window_days"})

        paper_cols = [
            "run", "group_id", "group_name", "training_window_days",
            "train_samples", "test_samples",
            "parking_duration_time_ms", "parking_duration_peak_memory_MB",
            "charging_capacity_time_ms", "charging_capacity_peak_memory_MB"
        ]
        paper_table = complexity_detail[[c for c in paper_cols if c in complexity_detail.columns]].copy()

        avg_row = {col: np.nan for col in paper_table.columns}
        if "group_name" in avg_row:
            avg_row["group_name"] = "Average"
        if "group_id" in avg_row:
            avg_row["group_id"] = "Average"
        if "training_window_days" in avg_row:
            avg_row["training_window_days"] = "Average"
        for col in paper_table.columns:
            if col not in ["run", "group_id", "group_name", "training_window_days"]:
                avg_row[col] = pd.to_numeric(paper_table[col], errors="coerce").mean()
        paper_table = pd.concat([paper_table, pd.DataFrame([avg_row])], ignore_index=True)

        complexity_detail.to_csv(OUTPUT_COMPLEXITY_DETAIL_CSV, index=False, encoding="utf-8-sig")
        complexity_summary.to_csv(OUTPUT_COMPLEXITY_SUMMARY_CSV, index=False, encoding="utf-8-sig")
        paper_table.to_csv(OUTPUT_COMPLEXITY_TABLE_CSV, index=False, encoding="utf-8-sig")
    ...
    try:
        with pd.ExcelWriter(OUTPUT_XLSX, engine="xlsxwriter") as writer:
            ...
            if not cost_summary.empty:
                cost_summary.to_excel(writer, index=False, sheet_name="computational_cost")
            if not complexity_detail.empty:
                complexity_detail.to_excel(writer, index=False, sheet_name="complexity_detail")
            if not complexity_summary.empty:
                complexity_summary.to_excel(writer, index=False, sheet_name="complexity_summary")
            if not paper_table.empty:
                paper_table.to_excel(writer, index=False, sheet_name="paper_table")
    except Exception:
    ...
    if not complexity_detail.empty:
        try:
            with pd.ExcelWriter(OUTPUT_COMPLEXITY_XLSX, engine="xlsxwriter") as writer:
                complexity_detail.to_excel(writer, index=False, sheet_name="complexity_detail")
                complexity_summary.to_excel(writer, index=False, sheet_name="complexity_summary")
                paper_table.to_excel(writer, index=False, sheet_name="paper_table")
        except Exception:
            with pd.ExcelWriter(OUTPUT_COMPLEXITY_XLSX, engine="openpyxl") as writer:
                complexity_detail.to_excel(writer, index=False, sheet_name="complexity_detail")
                complexity_summary.to_excel(writer, index=False, sheet_name="complexity_summary")
                paper_table.to_excel(writer, index=False, sheet_name="paper_table")
        ...
        for window in WINDOWS:
            ...
            for cid in retained_cluster_ids:
                ...
                print(
                    ...
                    f"peak_mem_P={out_row['peak_mem_parking_mb']:.2f}MB, peak_mem_K={out_row['peak_mem_kWh_mb']:.2f}MB"
                )
            ...
            overall_row = {
                ...
                "time_pt_mean_ms": float(cluster_df_metrics["time_pt_ms"].mean()),
                "time_kWh_mean_ms": float(cluster_df_metrics["time_kWh_ms"].mean()),
                "window_time_mean_ms": float(cluster_df_metrics["window_time_ms"].mean()),
                "peak_mem_pt_mean_mb": float(cluster_df_metrics["peak_mem_pt_mb"].mean()),
                "peak_mem_pt_max_mb": float(cluster_df_metrics["peak_mem_pt_mb"].max()),
                "peak_mem_kWh_mean_mb": float(cluster_df_metrics["peak_mem_kWh_mb"].mean()),
                "peak_mem_kWh_max_mb": float(cluster_df_metrics["peak_mem_kWh_mb"].max()),

                "mean_peak_mem_parking_mb": float(cluster_df_metrics["peak_mem_parking_mb"].mean()),
                "max_peak_mem_parking_mb": float(cluster_df_metrics["peak_mem_parking_mb"].max()),
                "mean_peak_mem_kWh_mb": float(cluster_df_metrics["peak_mem_kWh_mb"].mean()),
                "max_peak_mem_kWh_mb": float(cluster_df_metrics["peak_mem_kWh_mb"].max()),
            }
            ...
            print(
                ...
                f"peak_mem_P_mean={overall_row['mean_peak_mem_parking_mb']:.2f}MB, "
                f"peak_mem_K_mean={overall_row['mean_peak_mem_kWh_mb']:.2f}MB"
            )
