...
def main():
    ...
    base_dirs = [
        os.path.join(".", "results", "norway_pt_dual_dle_v16_complexity", "parking_time"),
        os.path.join(".", "results", "norway_pt_dual_dle_v16_complexity", "kWhDelivered"),
    ]
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            mem_monitor_p.start()
            target_start_p = time.perf_counter()
            ...
            train_time_p_ms = (end_p - start_p) * 1000

            mae_p = mae(y_p_te, pred_p_te)
            sm_p = smape(y_p_te, pred_p_te)

            target_end_p = time.perf_counter()
            time_p_ms = (target_end_p - target_start_p) * 1000
            mem_monitor_p.stop()

            peak_mem_p_mb = mem_monitor_p.peak_delta_mb
            peak_rss_p_mb = mem_monitor_p.peak_rss_mb

            folder1 = os.path.join(".", "results", "norway_pt_dual_dle_v16_complexity", "parking_time", str(run))
            ...
            mem_monitor_k.start()
            target_start_k = time.perf_counter()
            ...
            train_time_k_ms = (end_k - start_k) * 1000
            ...
            target_end_k = time.perf_counter()
            time_k_ms = (target_end_k - target_start_k) * 1000
            mem_monitor_k.stop()

            peak_mem_k_mb = mem_monitor_k.peak_delta_mb
            peak_rss_k_mb = mem_monitor_k.peak_rss_mb

            folder2 = os.path.join(".", "results", "norway_pt_dual_dle_v16_complexity", "kWhDelivered", str(run))
            ...
            print(f"  Window={w}: n_train={len(tr)}, n_test={len(te)}")
            print(
                f"    parking_time → MAE={mae_p:.3f}, SMAPE={sm_p:.3f}%, "
                f"rounds={rounds_p}, total_time={time_p_ms:.0f}ms, "
                f"peak_mem_delta={peak_mem_p_mb:.2f}MB"
            )
            print(
                f"    kWhDelivered → MAE={mae_k:.3f}, SMAPE={sm_k:.3f}%, "
                f"rounds={rounds_k}, total_time={time_k_ms:.0f}ms, "
                f"peak_mem_delta={peak_mem_k_mb:.2f}MB"
            )

            run_summary.append((
                w,
                mae_p, sm_p, rounds_p, time_p_ms, peak_mem_p_mb,
                mae_k, sm_k, rounds_k, time_k_ms, peak_mem_k_mb,
                t_window_ms
            ))
            metrics_all.append({
                "run": run,
                ...
                "train_time_pt_ms": train_time_p_ms,
                "peak_mem_pt_mb": peak_mem_p_mb,
                "peak_rss_pt_mb": peak_rss_p_mb,

                "mae_kWh": mae_k,
                "smape_kWh": sm_k,
                "rounds_kWh": rounds_k,
                "time_kWh_ms": time_k_ms,
                "train_time_kWh_ms": train_time_k_ms,
                "peak_mem_kWh_mb": peak_mem_k_mb,
                "peak_rss_kWh_mb": peak_rss_k_mb,
        ...
        print(f"\nResults for run {run}:")
        print(
            "window | MAE_pt | SMAPE_pt | rounds_pt | total_time_pt(ms) | peak_mem_pt(MB) | "
            "MAE_kWh | SMAPE_kWh | rounds_kWh | total_time_kWh(ms) | peak_mem_kWh(MB)"
        )
        for (w, mpt, spt, rp, tp, mp, mk, sk, rk, tk, mk_mem, _) in run_summary:
            print(
                f"{w:>6} | {mpt:6.3f} | {spt:7.3f}% | {rp:9d} | {tp:10.0f} | {mp:14.2f} | "
                f"{mk:7.3f} | {sk:9.3f}% | {rk:10d} | {tk:11.0f} | {mk_mem:15.2f}"
            )

    result_root = os.path.join(".", "results", "norway_pt_dual_dle_v16_complexity")
    os.makedirs(result_root, exist_ok=True)
    metrics_df = pd.DataFrame(metrics_all)
    metrics_path = os.path.join(result_root, "metrics_mixed_dynamic.csv")
    metrics_df.to_csv(metrics_path, index=False)

    cost_summary = metrics_df.groupby("window").agg(
        n_train_mean=("n_train", "mean"),
        n_test_mean=("n_test", "mean"),
        rounds_pt_mean=("rounds_pt", "mean"),
        time_pt_mean_ms=("time_pt_ms", "mean"),
        train_time_pt_mean_ms=("train_time_pt_ms", "mean"),
        peak_mem_pt_mean_mb=("peak_mem_pt_mb", "mean"),
        peak_mem_pt_max_mb=("peak_mem_pt_mb", "max"),

        rounds_kWh_mean=("rounds_kWh", "mean"),
        time_kWh_mean_ms=("time_kWh_ms", "mean"),
        train_time_kWh_mean_ms=("train_time_kWh_ms", "mean"),
        peak_mem_kWh_mean_mb=("peak_mem_kWh_mb", "mean"),
        peak_mem_kWh_max_mb=("peak_mem_kWh_mb", "max"),

        window_time_mean_ms=("window_time_ms", "mean")
    ).reset_index()

    complexity_table = cost_summary[[
        "window", "n_train_mean", "n_test_mean",
        "time_pt_mean_ms", "peak_mem_pt_mean_mb",
        "time_kWh_mean_ms", "peak_mem_kWh_mean_mb"
    ]].copy()
    complexity_table = complexity_table.rename(columns={
        "window": "training_window_days",
        "n_train_mean": "train_samples_mean",
        "n_test_mean": "test_samples_mean",
        "time_pt_mean_ms": "time_for_parking_duration_ms",
        "peak_mem_pt_mean_mb": "peak_memory_for_parking_duration_MB",
        "time_kWh_mean_ms": "time_for_charging_capacity_ms",
        "peak_mem_kWh_mean_mb": "peak_memory_for_charging_capacity_MB",
    })
    avg_row = {col: np.nan for col in complexity_table.columns}
    avg_row["training_window_days"] = "Average"
    for col in complexity_table.columns:
        if col != "training_window_days":
            avg_row[col] = pd.to_numeric(complexity_table[col], errors="coerce").mean()
    complexity_table = pd.concat([complexity_table, pd.DataFrame([avg_row])], ignore_index=True)

    complexity_detail = metrics_df[[
        "run", "window", "n_train", "n_test",
        "time_pt_ms", "peak_mem_pt_mb", "peak_rss_pt_mb",
        "time_kWh_ms", "peak_mem_kWh_mb", "peak_rss_kWh_mb",
        "window_time_ms", "mae_pt", "mae_kWh"
    ]].copy()
    complexity_detail = complexity_detail.rename(columns={
        "window": "training_window_days",
        ...
        "window_time_ms": "total_window_time_ms",
    })

    cost_path = os.path.join(result_root, "computational_cost_summary.csv")
    detail_path = os.path.join(result_root, "computational_cost_detail.csv")
    table_path = os.path.join(result_root, "computational_cost_table_for_paper.csv")
    xlsx_path = os.path.join(result_root, "norway_complexity_analysis.xlsx")
    cost_summary.to_csv(cost_path, index=False, encoding="utf-8-sig")
    complexity_detail.to_csv(detail_path, index=False, encoding="utf-8-sig")
    complexity_table.to_csv(table_path, index=False, encoding="utf-8-sig")
    try:
        with pd.ExcelWriter(xlsx_path, engine="xlsxwriter") as writer:
            metrics_df.to_excel(writer, index=False, sheet_name="metrics_raw")
            complexity_detail.to_excel(writer, index=False, sheet_name="complexity_detail")
            cost_summary.to_excel(writer, index=False, sheet_name="cost_summary")
            complexity_table.to_excel(writer, index=False, sheet_name="paper_table")
    except Exception:
    ...
    print(f"Computational cost summary saved to: {cost_path}")
    print(f"Computational cost detail saved to: {detail_path}")
    print(f"Paper table saved to: {table_path}")
    print(f"Complexity analysis workbook saved to: {xlsx_path}")
