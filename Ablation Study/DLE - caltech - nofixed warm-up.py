...
def main():
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            plt.tight_layout()
            folder1 = os.path.join(".", "results", "caltech_nogudingyure", "parking_time", str(run))
            ...
            ax.set_ylabel("Validation MAE")
            plt.tight_layout()
            folder2 = os.path.join(".", "results", "caltech_nogudingyure", "kWhDelivered", str(run))
    ...
    pd.DataFrame(metrics_all).to_csv("./results/caltech_nogudingyure/metrics_mixed_dynamic.csv", index=False)
