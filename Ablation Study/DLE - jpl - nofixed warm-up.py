...
def main():
    ...
    base_dirs = [
        os.path.join(".", "results", "jpl_ablation_no_warmup", "parking_time"),
        os.path.join(".", "results", "jpl_ablation_no_warmup", "kWhDelivered"),
    ]
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            prev_depth_p = None
            prev_gamma_p = None
            depth_p = PT_FIXED_DEPTH
            gamma_p = PT_FIXED_GAMMA

            start_p = time.perf_counter()
            for m in range(PT_MAX_ROUNDS):
                if m % PT_DYNAMIC_INTERVAL == 0:
                    if model_p is None:
            ...
            plt.tight_layout()
            folder1 = os.path.join(".", "results", "jpl_ablation_no_warmup", "parking_time", str(run))
            ...
            prev_depth_k = None
            prev_gamma_k = None
            depth_k = KWH_FIXED_DEPTH
            gamma_k = KWH_FIXED_GAMMA

            start_k = time.perf_counter()
            for m in range(KWH_MAX_ROUNDS):
                if m % KWH_DYNAMIC_INTERVAL == 0:
                    if model_k is None:
            ...
            ax.set_ylabel("Validation MAE")
            plt.tight_layout()
            folder2 = os.path.join(".", "results", "jpl_ablation_no_warmup", "kWhDelivered", str(run))
    ...
    pd.DataFrame(metrics_all).to_csv("./results/jpl_ablation_no_warmup/metrics_no_fixed_warmup.csv", index=False)
    print("\nAll runs complete. Results saved to metrics_no_fixed_warmup.csv")
