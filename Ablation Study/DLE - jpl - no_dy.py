...
def main():
    ...
    base_dirs = [
        os.path.join(".", "results", "jpl_ablation_no_dynamic", "parking_time"),
        os.path.join(".", "results", "jpl_ablation_no_dynamic", "kWhDelivered"),
    ]
    ...
    for run in range(1, n_runs + 1):
        ...
        for w in windows:
            ...
            params_p_fixed = {
                "tree_method": "hist",
                "eta": PT_FIXED_ETA,
                "max_depth": PT_FIXED_DEPTH,
                "gamma": PT_FIXED_GAMMA,
            ...
            for m in range(PT_MAX_ROUNDS):
                params_p = params_p_fixed.copy()
                params_p["seed"] = np.random.randint(1, 100000)
            ...
            plt.tight_layout()
            folder1 = os.path.join(".", "results", "jpl_ablation_no_dynamic", "parking_time", str(run))
            ...
            params_k_fixed = {
                "tree_method": "hist",
                "eta": KWH_FIXED_ETA,
                "max_depth": KWH_FIXED_DEPTH,
                "gamma": KWH_FIXED_GAMMA,
                "objective": "reg:pseudohubererror",
                "alpha": KWH_FIXED_ALPHA,
                "eval_metric": "mae",
                "verbosity": 0
            }
            ...
            ax.set_ylabel("Validation MAE")
            plt.tight_layout()
            folder2 = os.path.join(".", "results", "jpl_ablation_no_dynamic", "kWhDelivered", str(run))
    ...
    pd.DataFrame(metrics_all).to_csv("./results/jpl_ablation_no_dynamic/metrics_no_dynamic_tuning.csv", index=False)
    print("\nAll runs complete. Results saved to metrics_no_dynamic_tuning.csv")
