...
parse_dates = ['date']
df = pd.read_csv('GEFCom2014.csv', parse_dates=parse_dates, index_col='date')
...
MaximumLoad = MaximumLoad / 317.5
MinimumLoad = MinimumLoad / 317.5
Load = Load / 317.5
Temperature = Temperature / 97.7
...
iter_date = datetime.date(2005, 1, 1)            
...
input_1h = ctrl.Antecedent(np.arange(41, 391+1, 1), 'input_1h')
input_error = ctrl.Antecedent(np.arange(-63, 76+1, 1), 'input_error')
output_adjustment = ctrl.Consequent(np.arange(-5, 5+1, 1), 'output_adjustment')

# Designing the affiliation function
input_1h['low'] = fuzz.trimf(input_1h.universe, [41, 41, (41+391)/2])
input_1h['medium'] = fuzz.trimf(input_1h.universe, [95, (95+360)/2, 360])
input_1h['high'] = fuzz.trimf(input_1h.universe, [(41+391)/2, 391, 391])

input_error['low'] = fuzz.trimf(input_error.universe, [-63, -63, 0])
input_error['medium'] = fuzz.trimf(input_error.universe, [-40, 0, 50])
input_error['high'] = fuzz.trimf(input_error.universe, [0, 76, 76])

output_adjustment['negative'] = fuzz.trimf(output_adjustment.universe, [-5, -5, (-5+5)/2])
output_adjustment['zero'] = fuzz.trimf(output_adjustment.universe, [-1, 0, 1])
output_adjustment['positive'] = fuzz.trimf(output_adjustment.universe, [(-5+5)/2, 5, 5])
...
def adjust_fuzzy_rules(error_value):
    if error_value > 0:
        ...
        print(2)
        #Setting the affiliation function
        output_adjustment = ctrl.Consequent(np.arange(-2, 2+1, 1), 'output_adjustment')
        output_adjustment['negative'] = fuzz.trimf(output_adjustment.universe, [-2, -2, (-2+2)/2])
        output_adjustment['zero'] = fuzz.trimf(output_adjustment.universe, [-1, 0, 1])
        output_adjustment['positive'] = fuzz.trimf(output_adjustment.universe, [(-2+3)/2, 2, 2])
        #Rule
        rule1 = ctrl.Rule(input_error['low']&input_1h['low'], output_adjustment['zero'])
        rule2 = ctrl.Rule(input_error['low']&input_1h['medium'], output_adjustment['negative'])
        rule3 = ctrl.Rule(input_error['low']&input_1h['high'], output_adjustment['negative'])
...
for j in range(int(y_val_err.shape[0]/24)):
    ...
    for i in range(24):
        ...
        if error[j+i] < -63:
            error[j+i] = -63
        if pred_eval2_test_1h[j+i][50] > 391:
            pred_eval2_test_1h[j+i][50] = 391
        if pred_eval2_test_1h[j+i][50] < 41:
            pred_eval2_test_1h[j+i][50] = 41
