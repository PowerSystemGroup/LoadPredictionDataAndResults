    ...
    for j in range(i, NumberOfWeightsSaved):
        ...
        for k in range(NumberOfRepeatedTraining):
            for l in range(i,j+1):
                ...
                p[k*(j-i+1)+l-i,:] = pred.reshape(24*NumberOfTestSets) 
        pred_eval = np.mean(p, axis = 0)
        Output_test_eval = np.array(Output_test).transpose().reshape(24*NumberOfTestSets)
        mape = np.mean(np.divide(np.abs(Output_test_eval - pred_eval), Output_test_eval))
        mae = np.mean(np.abs(Output_test_eval - pred_eval))
        rmse = np.sqrt(np.mean(np.square(Output_test_eval - pred_eval)))
        loss_mape[i,j] = mape
        loss_mae[i,j] = mae
        loss_rmse[i,j] = rmse 