...
INPUT_XLSX = "jpl_station_scalability_splits.xlsx"
RESULT_ROOT = os.path.join(".", "results", "jpl_scalability")
...
def get_station_count_from_sheet(sheet_name):
    import re
    m = re.search(r"JPL_(\d+)_stations", str(sheet_name))
...
def preprocess_jpl_dataframe(raw_df, sheet_name):
    df0 = raw_df.copy()
...
def load_scalability_sheets(input_xlsx=INPUT_XLSX):
    ...
    if not sheet_names:
        sheet_names = [
            ...
            if str(s).startswith("JPL_") and str(s).endswith("_stations")
        ]
        sheet_names = sorted(
            sheet_names,
    ...
    if not sheet_names:
        raise ValueError(
            f"No sheet in the JPL_*_stations format was found in {input_path}."
        )
    ...
    for sheet in sheet_names:
        ...
        df0 = preprocess_jpl_dataframe(raw, sheet)
    ...
    for run in range(1, n_runs + 1):
        ...
        for sheet_name, df0 in df0_by_sheet.items():
            ...
            for w in windows:
                ...
                plt.tight_layout()
                folder1 = os.path.join(".", "results", "jpl_scalability", sheet_key, "parking_time", str(run))
                ...
                ax.set_ylabel("Validation MAE")
                plt.tight_layout()
                folder2 = os.path.join(".", "results", "jpl_scalability", sheet_key, "kWhDelivered", str(run))
