...
def build_station_quality_table(df):
    ...
    if tab.empty:
        raise RuntimeError("The site quality table cannot be generated. "
                           "Please check if there is any historical data prior to the test date.")
...
def apply_quality_filter(df, station_quality):
    ...
    if USE_STATION_SELECTION and cfg["top_stations"] is not None:
        ...
        if candidates.empty:
            print("Warning: No sites meet the minimum sample size requirement; "
                  "the process is being terminated without site screening.")
        else:
