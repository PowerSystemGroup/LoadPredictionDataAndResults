...
def get_base_config():
    return {
        "CSV_PATH": "jpl_test_data.csv",
        "RESULTS_ROOT": "./results/jpl_joint_sensitivity",
...
def get_run_plan(run_mode):
    if run_mode == "screening":
        ...
        raise ValueError("RUN_MODE can only be “screening” or “confirm”")
...
def get_given_baseline_metrics():
    windows = [30, 60, 120, 240, 360, 480]
    pt_smape = [9.39, 9.06, 8.7, 8.26, 8.6, 8.74]
    pt_mae = [0.97, 0.94, 0.92, 0.87, 0.89, 0.9]
    kwh_smape = [9.04, 9.3, 9.31, 8.55, 8.74, 8.91]
    kwh_mae = [2.79, 2.95, 2.99, 2.76, 2.85, 2.91]
...
def validate_formatted_output(formatted_df, sensitivity_plan):
    ...
    if problems:
        print("\n[WARNING] There are incomplete parameter sets in `sensitivity_raw_metrics.csv`:")
