...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"There are not enough historical samples; currently, there are only {len(hist)} entries, "
                         f"so an input sequence of length {encoder_length} cannot be constructed.")
...
def predict_array(model: nn.Module, X: np.ndarray, batch_size: int, device: torch.device):
    ...
    loader = DataLoader(SequenceForecastDataset(X, np.zeros((len(X),), dtype=np.float32)),
...
def main():
    os.makedirs("./JPL/results", exist_ok=True)
    os.makedirs("./JPL/results/Autoformer_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "JPL", "results", "Autoformer_strict", "parking_time"),
        os.path.join(".", "JPL", "results", "Autoformer_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "Autoformer_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
                    f"Window={w}  parking_time (Autoformer strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[AUTOFORMER STRICT ERROR - parking_time] Run={run}, Window={w}, Error: {e}")
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "Autoformer_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
                    f"Window={w}  kWhDelivered (Autoformer strict) Val MAE vs Epoch",
                )
            except Exception as e:
                print(f"[AUTOFORMER STRICT ERROR - kWhDelivered] Run={run}, Window={w}, Error: {e}")
    ...
    pd.DataFrame(metrics_all).to_csv("./JPL/results/Autoformer_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./JPL/results/Autoformer_strict/metrics_mixed_dynamic.csv")
