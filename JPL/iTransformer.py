...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"Insufficient historical samples; there are currently only {len(hist)} samples, "
...
def fit_itransformer_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"Too few training samples to construct an iTransformer sequence: {len(train_data)}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; the iTransformer cannot be trained")
...
def main():
    ...
    os.makedirs("./JPL/results/iTransformer_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "JPL", "results", "iTransformer_strict", "parking_time"),
        os.path.join(".", "JPL", "results", "iTransformer_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "iTransformer_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "iTransformer_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
    ...
    pd.DataFrame(metrics_all).to_csv("./JPL/results/iTransformer_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./JPL/results/iTransformer_strict/metrics_mixed_dynamic.csv")
