...
def fit_dlinear_model(
    ...
    if encoder_length < 4:
        raise ValueError(f"Too few training samples to construct an Dlinear sequence: {len(train_data)}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; the Dlinear cannot be trained")
...
def main():
    ...
    os.makedirs("./JPL/results/DLinear_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "JPL", "results", "DLinear_strict", "parking_time"),
        os.path.join(".", "JPL", "results", "DLinear_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "DLinear_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "DLinear_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
    ...
    pd.DataFrame(metrics_all).to_csv("./JPL/results/DLinear_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./JPL/results/DLinear_strict/metrics_mixed_dynamic.csv")
