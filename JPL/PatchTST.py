...
def build_single_inference_sequence(
    ...
    if len(hist) < encoder_length:
        raise ValueError(f"There are insufficient historical samples; currently, there are only {len(hist)} entries, "
                         f"so it is not possible to construct an input sequence of length {encoder_length}.")
...
def fit_patchtst_model(
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise ValueError("The training or validation set is empty; unable to train PatchTST.")
...
def main():
    ...
    os.makedirs("./JPL/results/PatchTST_strict", exist_ok=True)
    metrics_all = []
    ...
    base_dirs = [
        os.path.join(".", "JPL", "results", "PatchTST_strict", "parking_time"),
        os.path.join(".", "JPL", "results", "PatchTST_strict", "kWhDelivered"),
    ]
        ...
        for w in windows:
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "PatchTST_strict", "parking_time", run_str, f"convergence_parking_time_window_{w}.png"),
            ...
            try:
                ...
                save_curve_from_list(
                    ...
                    os.path.join(".", "JPL", "results", "PatchTST_strict", "kWhDelivered", run_str, f"convergence_kWh_time_window_{w}.png"),
    ...
    pd.DataFrame(metrics_all).to_csv("./JPL/results/PatchTST_strict/metrics_mixed_dynamic.csv", index=False)
    print("\nAll runs complete. Results saved to ./JPL/results/PatchTST_strict/metrics_mixed_dynamic.csv")
