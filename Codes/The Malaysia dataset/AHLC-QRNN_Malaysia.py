...
parse_dates = ['date']
df = pd.read_csv('Malaysia.csv', parse_dates=parse_dates, index_col='date')
...
MaximumLoad = MaximumLoad / 75447
MinimumLoad = MinimumLoad / 75447
Load = Load / 75447
...
number_of_days = 730
number_of_test_days = 61
number_of_train_days = 585
...
y_test = np.array(y1_test_new).reshape(24*61)*320
y_val = np.array(y1_val_new).reshape(24*61)*320
...
input_1h = ctrl.Antecedent(np.arange(65, 301+1, 1), 'input_1h')
input_error = ctrl.Antecedent(np.arange(-71, 72+1, 1), 'input_error')
output_adjustment = ctrl.Consequent(np.arange(-42, 37+1, 1), 'output_adjustment')

# Designing the affiliation function
input_1h['low'] = fuzz.trimf(input_1h.universe, [65, 65, (65+301)/2])
input_1h['medium'] = fuzz.trimf(input_1h.universe, [95, (95+292)/2, 292])
input_1h['high'] = fuzz.trimf(input_1h.universe, [(65+301)/2, 301, 301])

input_error['low'] = fuzz.trimf(input_error.universe, [-71, -71, 0])
input_error['medium'] = fuzz.trimf(input_error.universe, [-24, 0, 23])
input_error['high'] = fuzz.trimf(input_error.universe, [0, 72, 72])

output_adjustment['negative'] = fuzz.trimf(output_adjustment.universe, [-42, -42, (-42+37)/2])
output_adjustment['zero'] = fuzz.trimf(output_adjustment.universe, [-13, 0, 13])
output_adjustment['positive'] = fuzz.trimf(output_adjustment.universe, [(-42+37)/2, 37, 37])
...
def adjust_fuzzy_rules(error_value):
    if error_value > 0:
        ...
        print(2)
        #Setting the affiliation function
        output_adjustment = ctrl.Consequent(np.arange(-21, 18+1, 1), 'output_adjustment')
        output_adjustment['negative'] = fuzz.trimf(output_adjustment.universe, [-21, -21, (-42+18)/2])
        output_adjustment['zero'] = fuzz.trimf(output_adjustment.universe, [-13, 0, 13])
        output_adjustment['positive'] = fuzz.trimf(output_adjustment.universe, [(-21+18)/2, 18, 18])
        #Rule
        rule1 = ctrl.Rule(input_error['low']&input_1h['low'], output_adjustment['zero'])
        rule2 = ctrl.Rule(input_error['low']&input_1h['medium'], output_adjustment['negative'])
        rule3 = ctrl.Rule(input_error['low']&input_1h['high'], output_adjustment['negative'])
...
for j in range(int(y_val_err.shape[0]/24)):
    ...
    for i in range(24):
        ...
        if error[j+i] < -71:
            error[j+i] = -71
        if pred_eval2_test_1h[j+i][50] > 301:
            pred_eval2_test_1h[j+i][50] = 301
        if pred_eval2_test_1h[j+i][50] < 65:
            pred_eval2_test_1h[j+i][50] = 65
