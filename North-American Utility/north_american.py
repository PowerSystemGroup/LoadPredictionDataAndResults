# -*- coding: utf-8 -*-
"""
Created on 2023/9  north American utility
...
def get_layer_1(hour, input_Dd, input_Dw, input_Dm, input_Dr, input_Td, input_Tw, input_Tm, input_T, output_pre=[]):
    ...
    dense_concat1 = lstm_ResDD_layer(concat1)

    concat2 = concatenate([input_Dw, input_Tw])
    dense_concat2 = lstm_ResDD_layer(concat2)

    concat3 = concatenate([input_Dm, input_Tm])
    dense_concat3 = lstm_ResDD_layer(concat3)
    ...
    attention_output2 = Dropout(0.3)(dense_concat_mid)

    'FC1'
    dense_4 = DenseConcatDr(concatenate([dense_Dr, dense_concat_date_info_1]))

    attention_output4 = Dropout(0.3)(dense_4)


    'Lh under'
    concat = concatenate([attention_output2, attention_output4, input_T,dense_Dr])
...
for i in range(5):
    ...
    history_17 = model.fit(X_train_fit, Y_train_fit,validation_data=(X_val, Y_val), \
                           epochs=50, batch_size=BATCH_SIZE, callbacks=[scheduler3])

    model.save_weights('complete' + str(i + 1) + '17_weights.h5')
    print(str(i) + ' 17')

    history_18 = model.fit(X_train_fit, Y_train_fit, validation_data=(X_val, Y_val),\
                           epochs=50, batch_size=BATCH_SIZE, callbacks=[scheduler3])
    ...
    history_list.append(
        [history_1, history_2, history_3, history_4, history_5, history_6, history_7, history_8, history_9, history_10,
         history_11, history_12, history_13, history_14, history_15, history_16, history_17, history_18])
'''
